...
ggplot(data.frame(rt = seq(0, 2, length.out = 1000)), aes(rt)) +
  ...
  stat_function(fun = dLBA,
                ...
                args = list(response = 1, A=56, b= 65, t0 = 0.57, mean_v=c(61.4,68), sd_v=c(1,3))) +
  stat_function(fun = dLBA,
                aes(color = "Correct", linetype = "Off"),
                args = list(response = 2, A=56, b= 65, t0 = 0.57, mean_v=c(61.4,68), sd_v=c(1,3))) +
  geom_vline(xintercept = 0.8477883) +
  geom_vline(xintercept = 1.16) +
  scale_linetype_manual(breaks = c("True", "Off"), values = c(1, 2))

1 - (pLBA(rt = 1.16, response = 2, A=56, b= 65, t0 = 0.57, mean_v=c(61.4,68), sd_v=c(1,3)) + pLBA(rt = 1.16, response = 1, A=56, b= 65, t0 = 0.57, mean_v=c(61.4,68), sd_v=c(1,3)))

pLBA(rt = 10, response = 1, A=2, b= 4, t0 = .2, mean_v=c(3,4), sd_v=c(1,1.75)) + pLBA(rt = 10, response = 2, A=2, b= 4, t0 = .2, mean_v=c(3,4), sd_v=c(1,1.75))
...
both50known_burn9 <- get_pars(sMLBAboth50known, selection = "alpha",
                                stage = "burn", subject = 9)
both50known_adapt9 <- get_pars(sMLBAboth50known, selection = "alpha",
                                stage = "adapt", subject = 9)
both50known_sample9 <- get_pars(sMLBAboth50known, selection = "alpha",
                                 stage = "sample", subject = 9)
df_both50known_burn9 <-  data.frame()
df_both50known_adapt9 <-  data.frame()
df_both50known_sample9 <- data.frame()

for (par in names(parsLBA)) {
  for (chain in 1:3) {
    df_both50known_burn9 <- rbind(df_both50known_burn9, 
                                     data.frame(Parameter = par, Chain = chain,
                                                Value = c(both50known_burn9[[par]][[chain]]),
                                                Iteration = seq_along(
                                                  c(both50known_burn9[[par]][[chain]]
                                                )),
                                                True = parsLBA[[par]]))
    df_both50known_adapt9 <- rbind(df_both50known_adapt9, 
                                     data.frame(Parameter = par, Chain = chain,
                                                Value = c(both50known_adapt9[[par]][[chain]]),
                                                Iteration = seq_along(
                                                  c(both50known_adapt9[[par]][[chain]]
                                                )),
                                                True = parsLBA[[par]]))
    df_both50known_sample9 <- rbind(df_both50known_sample9, 
                                     data.frame(Parameter = par, Chain = chain,
                                                Value = c(both50known_sample9[[par]][[chain]]),
                                                Iteration = seq_along(
                                                  c(both50known_sample9[[par]][[chain]])),
                                                True = parsLBA[[par]]))
  }
}

df_both50known_burn9$Chain <- factor(df_both50known_burn9$Chain)
df_both50known_adapt9$Chain <- factor(df_both50known_adapt9$Chain)
df_both50known_sample9$Chain <- factor(df_both50known_sample9$Chain)

df_both50known_burn9 %>%
  ggplot(aes(x= Iteration, y = Value, color = Chain)) +
  facet_grid(Parameter ~ ., scales = "free_y", 
             labeller = labeller(Parameter = parlabeller, 
                                 .default = label_parsed)) +
