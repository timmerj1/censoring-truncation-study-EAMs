...
dadm <- attr(sMLBAupper50known, "design_list")[[1]]$model

# Copied from EMC2:::calc_ll_manager() () ----

c_name <- MLBA()$c_name
p_types <- MLBA()$p_types
designs <- list()
constants <- attr(sMLBAupper50known, "design_list")[[1]]$constants
n_trials = nrow(as.data.frame(sMLBAupper50known[[1]]$data$`10`))
parameter_indices <- list()

# calc_ll ----

EMC2:::calc_ll(t(as.matrix(parsLBA)), sMLBAupper50known[[1]]$data$`10`, constants = constants, designs = designs, type = c_name, p_types = p_types, min_ll = log(1e-10), group_idx = parameter_indices)

badstuff <- EMC2:::calc_ll(p_matrix = matrix(pars_EMCs_both$Median[pars_EMCs_both$EMC == sMLBAupper50known & pars_EMCs_both$id == 10], nrow = 1), data = as.data.frame(sMLBAupper50known[[1]]$data$`10`), constants = attr(sMLBAupper50known, "design_list")[[1]]$constants, designs = EMC2:::get_design_matrix(sMLBAupper50known), type = "MLBA", p_types, min_ll = log(1e-10), group_idx = list())

betterstuff <- exp(EMC2:::calc_ll(p_matrix = t(as.matrix(parsLBA)), data = as.data.frame(sMLBAupper50known[[1]]$data$`10`), constants = attr(sMLBAupper50known, "design_list")[[1]]$constants, designs = EMC2:::get_design_matrix(sMLBAupper50known), type = "MLBA", p_types, min_ll = log(1e-10), group_idx = list()))
...
#| fig-cap: "RMSE for the Log-Normal Race Model (LNR)"
#| fig-alt: ""
#| apa-note: "Something something"
...
EMCs_both_RMSEs %>%
  ...
  summarise(mean = mean(RMSE), 
            RMSE_CI = qt(.975, 10 * 8 - 8) * sd(RMSE) / sqrt(n()),
            .by = c(response_known, tail, percentage, censoring)) %>%
  ggplot(aes(percentage, mean, color = censoring)) +
  facet_grid(response_known ~ tail) +
  geom_point() +
  geom_path(aes(group = censoring)) +
  geom_errorbar(aes(ymin = mean - RMSE_CI, ymax = mean + RMSE_CI)) +
  labs(fill = "Missing") +
  theme_classic() +
  scale_y_continuous(n.breaks = 3)
...
#| fig-cap: "MAE for the Log-Normal Race Model (LNR)"
#| fig-alt: ""
#| apa-note: "Something something"
...
#| fig-cap: "RMSE for the Racing Diffusion Model (RDM)"
#| fig-alt: ""
#| apa-note: "Something something"
